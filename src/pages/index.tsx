      ...
      <main className="border-b-4 border-red pb-14">
            ...
            {technologies?.map((technology, index) => {
              return (
                <SwiperSlide key={index}>
                  <CardTec title={technology.title} image={technology.iconUrl} />
                </SwiperSlide>
              )
            })}
        ...
        <section id='projetos' className="container mx-auto mt-48 px-4">
          <div className="md:flex">
            ...
            <Swiper
              slidesPerView={width < 1024 ? 1.15 : 2}
              spaceBetween={15}
              loop={true}
              navigation
              centeredSlides={true}
          ...
          <a
            className="h-16 bg-red px-6 text-white uppercase font-bold flex items-center justify-center rounded mt-14 mx-auto max-w-xs"
            href="https://github.com/sanderpaniago"
            target='_blank'
          >Acessar Github</a>
        </section>
      </main>
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const technologies = data.allTecologiass.edges[0].node.technology.map(item => {
    return {
  ...
  const projects = data.allProjects.edges[0].node.project.map(item => {
    return {
      image: item.image.url,
      title: item.title[0].text,
      description: item.description[0].text,
      linkRepository: item.link_repository[0].text,
      linkDemo: item.link_demo[0].text
    }
  })
